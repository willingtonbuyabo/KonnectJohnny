          ...
          <div className="flex-1 flex flex-col bg-gradient-to-tr from-pink-500/20 via-yellow-500/10 to-violet-500/20 text-brand-cream font-sans relative overflow-hidden pt-6">
            ...
            <div className="h-12 border-b border-pink-500/30 px-4 flex items-center justify-between bg-black/40 backdrop-blur-md">
              <div className="flex items-center gap-1.5">
                <span className="w-6 h-6 border-2 border-brand-gold flex items-center justify-center text-white text-[9px] font-bold font-serif rounded-full bg-gradient-to-tr from-pink-500 to-yellow-500">KJ</span>
                <span className="text-[10px] tracking-widest font-serif font-black text-transparent bg-clip-text bg-gradient-to-r from-pink-400 via-yellow-400 to-violet-400">KONNECTJOHNNY</span>
              </div>
              <div className="text-[8px] font-mono text-pink-400 bg-pink-500/15 border border-pink-500/30 px-2 py-0.5 rounded-full flex items-center gap-1">
            ...
            <div className="flex-1 p-3 flex flex-col justify-between relative">
              ...
              <AnimatePresence mode="wait">
                {activeDemoProfile ? (
                  ...
                  >
                    ...
                    <div className="p-4 relative z-20 space-y-1.5 text-left bg-gradient-to-t from-black via-black/85 to-transparent">
                      <div className="flex items-baseline gap-1.5">
                        <h3 className="font-serif text-xl font-black text-transparent bg-clip-text bg-gradient-to-r from-pink-400 via-yellow-300 to-violet-300">{activeDemoProfile.name}</h3>
                        <span className="text-xs font-bold text-white">{activeDemoProfile.age}</span>
                      </div>

                      {/* Location */}
                      <p className="text-[10px] text-yellow-300 flex items-center gap-1 font-bold">
                        <MapPin className="w-3 h-3 text-pink-400" />
                        {activeDemoProfile.location_name}
                      </p>

                      {/* Bio */}
                      <p className="text-[10px] text-gray-100 font-medium line-clamp-2 leading-relaxed">
                        {activeDemoProfile.bio}
                      </p>

                      {/* Bio Interest tags - Vibrant custom colors */}
                      <div className="flex flex-wrap gap-1 pt-1">
                        {activeDemoProfile.interests.slice(0, 3).map((interest, idx) => {
                          const tagColors = [
                          ...
                          return (
                            <span key={interest} className={`text-[8px] px-1.5 py-0.5 rounded-full border font-bold ${colorClass}`}>
                              {interest}
                            </span>
                          );
                        })}
              ...
              {/* Simulated action buttons with hyper-vibrancy */}
              {activeDemoProfile && (
                <div className="flex justify-center gap-4 py-2 mt-2">
                  <button 
                    onClick={() => handleDemoSwipe("left")}
                    className="w-10 h-10 rounded-full bg-black/40 border border-pink-500/30 text-pink-400 flex items-center justify-center active:scale-90 transition-all hover:bg-pink-500/10"
                  >
                    <X className="w-5 h-5 stroke-[3]" />
                  </button>
                  <button 
                    onClick={() => handleDemoSwipe("right")}
                    className="w-12 h-12 rounded-full bg-gradient-to-tr from-pink-500 via-yellow-400 to-violet-500 text-black flex items-center justify-center active:scale-90 transition-all hover:brightness-110 shadow-lg shadow-pink-500/30"
                  >
                    <Heart className="w-6 h-6 fill-black text-black stroke-[3]" />
                  </button>
                </div>
              )}
  ...
  return (
    <div className="w-full max-w-6xl mx-auto px-4 md:px-8 py-4 md:py-12 flex flex-col lg:flex-row items-center gap-12 lg:gap-16 min-h-[calc(100vh-140px)] select-none">
      ...
      <div className="flex-1 flex flex-col space-y-8 text-left max-w-xl">
        ...
        {/* 1. Header & Title Block (Order 1 on mobile) */}
        ...
        {/* 3. Welcome Paragraph (Order 3 on mobile) */}
        ...
        {/* 4. Feature grid (Order 4 on mobile) */}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2 order-4">
        ...
        {/* 5. Primary Call to Actions (Order 5 on mobile) */}
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 pt-4 order-5">
          <button
            onClick={() => onStartAuth(true)}
        ...
        {isDemoMode && (
          <p className="text-[10px] text-brand-gold-muted italic flex items-center gap-1 order-6">
